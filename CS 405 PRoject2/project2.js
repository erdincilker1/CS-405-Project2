...
class MeshDrawer {
	...
	setSpecularLight(specular) {
        ...
        gl.uniform1f(this.specularIntensityLoc, specular);
    }

    // Method to set shininess factor
    setShininess(shiny) {
        gl.useProgram(this.prog);
        gl.uniform1f(this.shininessLoc, shiny);
    }
	...
	draw(trans) {
		...
		gl.uniform3f(gl.getUniformLocation(this.prog, 'lightPos'), lightX, lightY, 1.0);
	...
	enableLighting(show) {
		gl.useProgram(this.prog);
		gl.uniform1i(gl.getUniformLocation(this.prog, 'enableLighting'), show);
	}
...
const meshFS = `
			...
			void main()
			{
				...
				if (showTex && enableLighting) {
					...
					vec4 texColor = texture2D(tex, v_texCoord);
					gl_FragColor = vec4(texColor.rgb * (ambient + diff), texColor.a);
